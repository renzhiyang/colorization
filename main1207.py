    ...
    try:
        for step in range(MAX_STEP):
            ...
            if step % 2000 == 0:
                ...
                axes1.scatter(ab[:, :, 0], ab[:, :, 1],alpha=0.5, s=8)
                plt.xlabel('a')
                plt.ylabel('b')
                plt.title('input images')

                axes2 = plt.subplot(222)
                axes2.scatter(ab_out[:, :, 0], ab_out[:, :, 1],alpha=0.5, s=8)
                plt.xlabel('a')
                plt.ylabel('b')
                plt.title('output images')

                axes3 = plt.subplot(223)
                axes3.scatter(ab_index[:, :, 0], ab_index[:, :, 1], alpha=0.5, s=8)
                plt.xlabel('a')
                plt.ylabel('b')
                plt.title('index images')

                axes4 = plt.subplot(224)
                part1 = axes4.scatter(ab[:, :, 0], ab[:, :, 1], alpha=0.5, label='image_in', s=8)
                part2 = axes4.scatter(ab_out[:, :, 0], ab_out[:, :, 1], alpha=0.5, label='image_out', c = 'r', s=8)
                part3 = axes4.scatter(ab_index[:, :, 0], ab_index[:, :, 1], alpha=0.5, label='image_index', c='g', s=8)
                plt.xlabel('a')
                plt.ylabel('b')
